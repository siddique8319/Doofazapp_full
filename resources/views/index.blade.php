...
{{--                              <li><router-link  to="/shopList">  <span>Instant Shop List</span></router-link></li>  --}}
          ...
          <div class="st-content-inner">
            <div class="container-fluid">
                <div v-if="!$route.meta.hideDashboard" >
                  @if(Auth::user()->type==1)
                  <div class="row" data-toggle="isotope" style="position: relative; height: 429.1px;">
                        <div class="item col-md-4 col-sm-6 col-xs-12" style="position: absolute; left: 264px; top: 0px;">
                            <div class="panel panel-default widget-user-1 text-center">
                                <div class="avatar">
                                      <img src="{{asset('frontendadmin/images/images1.png')}}" class="img-circle" style="height:150px">
                                      <h3  style="color:skyblue">{{Auth::user()->name}}</h3>
                                      <hr>
                                      <h4>{{Auth::user()->mobile}}</h4>
                                      <hr>
                                      @foreach($designation as $designations)
                                      @if($designations->designationId==Auth::user()->designation)
                                      <h4>{{$designations->designationName}}</h4>
                                      @endif
                                      @endforeach
                                  </div>
                              </div>
                          </div>
                        <div class="item col-md-4 col-sm-6 col-xs-12" style="position: absolute; left: 528px; top: 0px;">
                          <div class="panel panel-default">
                              <div class="panel-heading" style="background-color: rgba(100, 136, 192, 0.22);">
                                <div class="media">
                                    <Center><h3 style="color:#04171f">Statement</h3></Center>
                                    <hr>
                                    <h5 style="color:#04171f">Total Earn:100</h5>
                                    <hr>
                                    <h5  style="color:#04171f">Total Direct Sale:20</h5>
                                    <hr>
                                    <h5 style="color:#04171f">Active Shop:10</h5>
                                    <hr>
                                    <h5  style="color:#04171f">Total Team sale:20</h5>
                                    <hr>
                                    <h5  style="color:#04171f">Total Withdraw:80</h5>
                                    <hr>
                                    <h5  style="color:#04171f">Available Balance:20</h5>
                                </div>
                                </div>
                          </div>
                        </div>
                  </div>
                  @foreach($incomeType as $incomeTypes)
                  <div class="item col-xs-12 col-md-10" >
                    <h3 class="ribbon-heading ribbon-default top-left-right">{{$incomeTypes->incomeTypeName}}-Reveneu</h3>
                  </div>
                  @php
                       $income=DB::table('incomes')
                       ->where('incomeTypeId',$incomeTypes->incomeTypeId)
                       ->get();
                  @endphp
                  <div class="row clearfix">
                      ...
                      @foreach($income as $incomes)
                          <div class="col-lg-3 col-md-6 col-sm-12">
                            <div class="widget">
                                <div class="widget-body" style="background-color:white;height: 95px;">
                                      <div style="padding: 10px; text-align: center; margin: 18px;">
                                          <div class="dashboardIcon">
                                            <i class="fa {{$incomes->icon}} iconSize" style="font-size:22px"></i>
                                          </div>
                                          <div class="iconText">
                                              <h2>৳ {{$incomes->amount}}</h2>
                                              <h5>{{$incomes->incomeName}}</h5>
                                          </div>
                                      </div>
                                </div>
                            </div>
                  ...
                  @endforeach
                  <div class="item col-xs-12 col-md-10" >
                      <h3 class="ribbon-heading ribbon-default top-left-right">Shop Renew</h3>
                        <div class="panel panel-default">
                 ...
                 @else
                  ...
                  <div class="row clearfix">
                      ...
                      @foreach($income as $incomes)
                        @php
                            $incomeShow=DB::table('account_information')
                            ->where('customerId',Auth::user()->id)
                            ->where('incomeType',$incomeTypes->incomeTypeId)
                            ->where('salesType',$incomes->id)
                            ->get();
                        @endphp
                       @foreach($incomeShow as $incomeShows)
                          <a href="more&{{$incomeShows->incomeType}}&{{$incomeShows->salesType}}">
                          <div class="col-lg-3 col-md-6 col-sm-12">
                            <div class="widget">
                                <div class="widget-body" style="background-color:white;height: 95px;">
                                      <div style="padding: 10px; text-align: center; margin: 18px;">
                                          ...
                                          <div class="iconText">
                                              <h2>৳ {{number_format($incomeShows->balance)}}</h2>
                                              @if($incomeShows->salesType==$incomes->id)
                                              <h5>{{$incomes->incomeName}}</h5>
                                              @endif
                                          </div>
                                      </div>
