                        ...
                        <div class="row">
                           <div class="col-md-6">  
                             <div class="form-group" :class="{ 'has-error': form.errors.has('type') }" >
                                <label for="exampleFormControlSelect1">Select Designation</label>
                                 <select class="form-control"   v-model="form.type" id="exampleFormControlSelect1" name="type" :class="{ 'is-invalid': form.errors.has('type') }" >
                                   <option  v-for="designation in designations" :key="designation.designationId" v-bind:value=designation.designationId>{{designation.designationName}}</option>
                                 </select>
...
<script>
    ...
    methods: {
        add(){
           this.form.post('/designationCondition').then(response => {
              this.form.reset(); 
              this.condition(); 
              Toast.fire({
                      icon: 'success',
