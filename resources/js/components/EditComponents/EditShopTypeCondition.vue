...
<script>
    ...
    methods: {
        editCondition(){
            axios.get(`/shopTypeCondition/${this.$route.params.id}/edit`).then(res =>{
                this.form.fill(res.data)
              })
         }, 
         updateCondition(){
            axios.put(`/shopTypeCondition/${this.$route.params.id}` ,this.form).then(res =>{
              this.$router.push({name: 'shopTypeCondition'});
               Toast.fire({
                  icon: 'success',
