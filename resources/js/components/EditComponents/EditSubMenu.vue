                       ...
                       <div class="row">
                          <div class="form-group"  >
                             <label for="exampleFormControlSelect1">Select Menu</label>
                                <select class="form-control"   v-model="form.menuId" name="menuId" id="exampleFormControlSelect1" >
                        ...
                        <table data-toggle="data-table" class="table table-responsive" cellspacing="1" width="200%">
                          ...
                          <tbody>
                            <tr v-for="(subMenu, index ) in subMenus" :key="subMenu.subMenuId">
                               <td>{{ index+1 }}</td>
                               <td v-if="subMenu.menu_relation">{{ subMenu.menu_relation.menuName}}</td>
...
<script>
    ...
    methods: {
         edit(){
             axios.get(`/subMenu/${this.$route.params.id}/edit`).then(res =>{
                 this.form.fill(res.data)
               })
         },  
        update(){
            axios.put(`/subMenu/${this.$route.params.id}`,this.form).then(res =>{
